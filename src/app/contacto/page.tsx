import type { Metadata } from 'next'
import Breadcrumbs from '@/components/Breadcrumbs'

export const metadata: Metadata = {
  title: 'Contacto',
  description: 'Contacta con AbrazoGigante para alquilar disfraces hinchables en Valencia. Reserva por teléfono o WhatsApp.',
  alternates: { canonical: 'https://abrazogigante.es/contacto' },
}

const PHONE = '+34602042056'
const PHONE_DISPLAY = '+34 602 042 056'
const WHATSAPP_URL = `https://wa.me/${PHONE}`

export default function ContactoPage() {
  return (
    <div suppressHydrationWarning>
      <main className="min-h-screen pt-10 pb-32 px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <Breadcrumbs />
          <h1 className="section-title mb-8">Contacto</h1>

          <section className="max-w-lg mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-extrabold uppercase mb-2" style={{color: '#1A1A1A'}}>Reserva ahora</h2>
            <p className="text-sm md:text-base mb-8 font-semibold text-gray-500">Llámanos y en minutos tienes todo listo</p>

            <div className="space-y-6">
            <a
              href={`tel:${PHONE}`}
              className="flex items-center gap-4 w-full px-6 py-5 rounded-xl border-2 border-gray-200 hover:border-[#d0632a] transition-colors bg-white"
            >
              <div className="w-12 h-12 rounded-full bg-[#d0632a] flex items-center justify-center shrink-0">
                <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
                </svg>
              </div>
              <div className="text-left">
                <p className="font-bold text-gray-800">Llamar ahora</p>
                <p className="text-sm text-gray-500">{PHONE_DISPLAY}</p>
              </div>
            </a>

            <a
              href={WHATSAPP_URL}
              rel="noopener noreferrer"
              className="flex items-center gap-4 w-full px-6 py-5 rounded-xl border-2 border-gray-200 hover:border-[#25D366] transition-colors bg-white"
            >
              <div className="w-12 h-12 rounded-full bg-[#25D366] flex items-center justify-center shrink-0">
                <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/>
                </svg>
              </div>
              <div className="text-left">
                <p className="font-bold text-gray-800">WhatsApp</p>
                <p className="text-sm text-gray-500">Escribe un mensaje</p>
              </div>
            </a>
            </div>
          </section>
        </div>
      </main>
    </div>
  )
}
